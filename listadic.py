...
def atualizar(carros):
    atualizar = forcaopcao("Qual carro você deseja atualizar?", carros[ke])
    indice_atualizar = achaindice(carros['nome'],atualizar)
    for key in carros.keys():
...
while True:
    menu = forcaopcao('Olá, Escolha uma seção: ', ['Apagar', 'Registrar', 'Atualizar', 'Listar'])
    if menu == 'Apagar':
        remover = forcaopcao("qual carne vc quer remover? ", acougue['carne'])
        indiceremover = achaindice(acougue['carne'], remover)
        for key in acougue.keys():
            acougue[key].pop(indiceremover)
    elif menu == 'Registrar':
        cadastrar(acougue)
    elif menu == 'Atualizar':
        atualizar(acougue)
